...
public class CrashReportGraphic extends VBox {
    public CrashReportGraphic(final CrashReport report) {
        super();
        this.getChildren().addAll(CrashReportGraphic.dateLabel(report), CrashReportGraphic.descriptionLabel(report));
    }

    private static Label dateLabel(final CrashReport report) {
        final String dateString = report.getDateTime().format(DateTimeFormatter.ISO_TIME);
        final Label label;
    ...
    private static Label descriptionLabel(final CrashReport report) {
        String processString = report.getRegisteredName();
        if (null != processString && processString.isEmpty()) {
